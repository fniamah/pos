...
<input type="hidden"  value="<?php echo $stfID; ?>" id="duserid"/>
	...
	<div id="container">
                                ...
                                <div class="widget-content no-padding">
                                    <table class="table table-striped table-bordered table-hover table-checkable table-responsive datatable">
                                        ...
                                        <tr>
                                            ...
                                            <td><a class="btn btn-sm btn-primary" onclick="editcategory(<?php echo $cid; ?>,'<?php echo $cname; ?>','<?php echo $status; ?>')">Update</a></td>
                                        </tr>
                                        <?php } ?>
                                        </tbody>
                                    </table>
        ...
        <?php $conn->close($dbconnection);}elseif(isset($_GET['salesreport'])){
            ...
            <?php $conn->close($dbconnection);}elseif(isset($_GET['invoicereport'])){
            $conn=new Db_connect;
            $dbconnection=$conn->conn();
            $shop = $_GET['invoicereport'];
            $user = $_GET['salesofficer'];
            $sdate = $_GET['sdate'];
            $startdate = $sdate." 00:00:00";
            $edate = $_GET['edate'];
            ...
            $sel = "";
            if($shop == "All" && $user == "All"){
                $sel = "SELECT * FROM invoice_payment WHERE paydate BETWEEN '$startdate' AND '$enddate' ORDER BY paydate ASC";
            }elseif($shop == "All" && $user != "All"){
                $sel = "SELECT * FROM invoice_payment WHERE userid ='$user' AND paydate BETWEEN '$startdate' AND '$enddate' ORDER BY paydate ASC";
            }elseif($shop != "All" && $user == "All"){
                $sel = "SELECT p.invoiceid, p.amount, p.paydate, p.userid  FROM invoice_payment p INNER JOIN customer_invoice c ON p.invoiceid = c.invoiceid WHERE c.shop ='$shop' AND p.paydate BETWEEN '$startdate' AND '$enddate' ORDER BY paydate ASC";
            }else{
                $sel = "SELECT p.invoiceid, p.amount, p.paydate, p.userid  FROM invoice_payment p INNER JOIN customer_invoice c ON p.invoiceid = c.invoiceid WHERE c.shop ='$shop' p.userid='$user' AND p.paydate BETWEEN '$startdate' AND '$enddate' ORDER BY paydate ASC";
            }
            $selrun = $conn->query($dbconnection,$sel);
            $totcount = $conn->sqlnum($selrun);
            ?>
                ...
                <div class="container">
                    ...
                    <div class="crumbs">
                        <ul id="breadcrumbs" class="breadcrumb">
                            ...
                            <li class="current">
                                <a title="">Invoice Payments Report</a>
                            </li>
                        </ul>
                    </div>
                    <!-- /Breadcrumbs line -->
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="widget-content no-padding">
                                <table class="table table-striped table-bordered table-hover table-checkable table-responsive">
                                    <thead>
                                    <tr><th colspan="9"><p style="color: #ff630d; font-weight: bold; font-size: large; text-align: center">INVOICE PAYMENTS REPORT</p></th></tr>
                                    <tr>
                                        <th>#</th>
                                        <th data-class="expand">INVOICE ID</th>
                                        <th>AMOUNT PAID</th>
                                        <th data-hide="phone">PAYMENT DATE</th>
                                        <th data-hide="phone">COLLECTED BY</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php
                                    ...
                                    <?php }else{
                                        while($data = $conn->fetch($selrun)){
                                            ...
                                            <tr>
                                                <td><?php echo $count; ?></td>
                                                <td><a href="dashboard.php?viewinvoice=<?php echo $data['invoiceid']; ?>&branch=<?php echo $branch; ?>"><?php echo $invid; ?></a></td>
                                                <td><?php echo $data['amount']; ?></td>
                                                <td><?php echo date("d M, Y H:i:s", strtotime(date($data['paydate']))); ?></td>
                                                <td><?php echo getstaff($data['userid']); ?></td>
                                            </tr>
                                        <?php }} ?>
                                    <tr style="color: #ff630d; font-weight: bold; font-size: medium">
                                            <td>&nbsp;</td><td>TOTAL (GHS)</td><td><?php echo number_format($totalamount, 2, '.', ','); ?></td>&nbsp;
                                    </tr>
                                    </tbody>
                                </table>
                            </div>
        ...
        <?php $conn->close($dbconnection); }elseif(isset($_GET['customers'])){
            ...
            <?php $conn->close($dbconnection);}elseif(isset($_GET['quotations'])){
                ...
                <?php $conn->close($dbconnection);}elseif(isset($_GET['shop_transfer'])){
                    $conn=new Db_connect;
                    $dbconnection=$conn->conn();
                    echo $sel = "SELECT * FROM transfer_details WHERE fromshop = '$branch' ORDER BY tstamp DESC";
                    $selrun = $conn->query($dbconnection,$sel);
                    $selnum = $conn->sqlnum($selrun);
                    ?>
                    <div id="content">
                        <div class="container">
                            ...
                            <div class="crumbs">
                                <ul id="breadcrumbs" class="breadcrumb">
                                    ...
                                    <li class="current">
                                        <a title="">Shop Outbound Transfers</a>
                                    </li>
                                </ul>
                            </div>
                            <!-- /Breadcrumbs line -->
                            ...
                            <div class="row row-bg"></div> <!-- /.row -->
                            <div class="row">
                                <div class="col-md-12" style="padding: 10px;" align="right"><a  data-toggle="modal" href="#shoptransfer" class="btn btn-danger btn-lg">Transfer Product </a></div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="widget-content no-padding">
                                        <table class="table table-striped table-bordered table-hover table-checkable table-responsive dataTable">
                                            <thead>
                                            <tr><th colspan="4"><p style="text-align: center; font-weight: bold; font-size: large;">SHOP OUTBOUND TRANSFERS</p></th></tr>
                                            <tr>
                                                <th>#</th>
                                                <!--<th>Image</th>-->
                                                <th>Transfer #</th>
                                            ...
                                            while($data = $conn->fetch($selrun)){
                                                ...
                                                <tr>
                                                    <td><?php echo $count; ?></td>
                                                    <td><a href="dashboard.php?transferid=<?php echo $transid; ?>&branch=<?php echo $branch; ?>"><?php echo $transid; ?></a></td>
                                                    <td><?php echo getbranch($data['shop']); ?></td>
                ...
                <?php $conn->close($dbconnection);}elseif(isset($_GET['pending_transfers'])){
                    $conn=new Db_connect;
                    $dbconnection=$conn->conn();
                    $sel = "SELECT * FROM transfer_details WHERE shop = '$branch' ORDER BY tstamp ASC";
                    $selrun = $conn->query($dbconnection,$sel);
                    $selnum = $conn->sqlnum($selrun);
                    ?>
                    <div id="content">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="widget-content no-padding">
                                        <table class="table table-striped table-bordered table-hover table-checkable table-responsive dataTable">
                                            <thead>
                                            <tr><th colspan="4"><p style="text-align: center; font-weight: bold; font-size: large;">INBOUND TRANSFERS</p></th></tr>
                                            <tr>
                                                <th>#</th>
                                                <!--<th>Image</th>-->
                                                <th>Transfer #</th>
                                            ...
                                            while($data = $conn->fetch($selrun)){
                                                ...
                                                <tr>
                                                    <td><?php echo $count; ?></td>
                                                    <td><a href="dashboard.php?transferid=<?php echo $transid; ?>&branch=<?php echo $branch; ?>&type=shop"><?php echo $transid; ?></a></td>
                                                    <td><?php echo getbranch($data['fromshop']); ?></td>
                                                    <td><?php echo $data['tstamp']; ?></td>
                                                    <td><?php echo $data['status']; ?></td>
                                                </tr>
                                            <?php }}else{?>
                    ...
                    <?php $conn->close($dbconnection);}elseif(isset($_GET['inbound_pending_transfers'])){
                    $conn=new Db_connect;
                    $dbconnection=$conn->conn();
                    $sel = "SELECT * FROM transfer_details WHERE shop = 'WH' ORDER BY tstamp ASC";
                    $selrun = $conn->query($dbconnection,$sel);
                    $selnum = $conn->sqlnum($selrun);
                    ?>
                    <div id="content">
                        <div class="container">
                            ...
                            <div class="page-header">
                                ...
                                <ul class="page-stats">
                                    <li>
                                        <div class="summary">
                                            <span>Warehouse Inbound Transfers</span>
                                            <h3 align="center"><?php echo $selnum; ?></h3>
                                        </div>
                                    </li>
                                </ul>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="widget-content no-padding">
                                        <table class="table table-striped table-bordered table-hover table-checkable table-responsive dataTable">
                                            <thead>
                                            <tr><th colspan="4"><p style="text-align: center; font-weight: bold; font-size: large;">WAREHOUSE INBOUND TRANSFERS</p></th></tr>
                                            <tr>
                                                <th>#</th>
                                                <!--<th>Image</th>-->
                                                <th>Transfer #</th>
                    ...
                    $desc = "";
                    if($shop == "All"){
                        $sel = "SELECT * FROM transfer_details WHERE transdate BETWEEN '$sdate' AND '$edate' AND fromshop='WH'";
                        $desc = "Product Transfers To All Shops From ".$sdate." To ".$edate;
                    }else{
                        $sel = "SELECT * FROM transfer_details WHERE shop = '$shop' AND transdate BETWEEN '$sdate' AND '$edate' AND fromshop='WH'";
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="widget-content no-padding">
                                        <table class="table table-striped table-bordered table-hover table-checkable table-responsive dataTable">
                                            <thead>
                                            <tr><th colspan="4"><p style="text-align: center; font-weight: bold; font-size: large;">OUTBOUND TRANSFERS FROM <?php echo $sdate." TO ".$edate; ?></p></th></tr>
                                            <tr>
                                                <th>#</th>
                                                <!--<th>Image</th>-->
                                                <th>Transfer #</th>
                                            ...
                                            while($data = $conn->fetch($selrun)){
                                                ...
                                                <tr>
                                                    <td><?php echo $count; ?></td>
                                                    <td><a href="dashboard.php?transferid=<?php echo $transid; ?>&branch=<?php echo $branch; ?>&type=wh"><?php echo $transid; ?></a></td>
                                                    <td><?php echo getbranch($data['shop']); ?></td>
                                                    <td><?php echo $data['tstamp']; ?></td>
                                                    <td><?php echo $data['status']; ?></td>
                                                </tr>
            ...
            <?php $conn->close($dbconnection);}elseif(isset($_GET['transferid'])){
                    $conn=new Db_connect;
                    $dbconnection=$conn->conn();
                    $invid = $_GET['transferid'];
                    $type = $_GET['type'];
                    $status = "";
                    if(isset($_GET['status'])){
                        $status = $_GET['status'];
                    }
                    $sel = "SELECT * FROM transfer_details WHERE transid = '$invid'";
                    $selrun = $conn->query($dbconnection,$sel);
                    $seldata = $conn->fetch($selrun);
                    $sender = $seldata['transferby'];
                    $sendershop = $seldata['fromshop'];
                    ?>
                    <div id="content">
                        <div class="container">
                            <!-- Breadcrumbs line -->
                            ...
                            <div class="row row-bg">
                                ...
                                <div class="alert alert-success fade in">
                                    ...
                                    <strong>Success!</strong> Products Have Been Transferred Successfully.
                                </div>
                                <?php }elseif($status == "rejected"){ ?>
                                    <div class="alert alert-danger fade in">
                                        <i class="icon-remove close" data-dismiss="alert"></i>
                                        <strong>Success!</strong> Products Have Been Rejected Successfully.
                                    </div>
                                <?php } ?>
                            </div> <!-- /.row -->
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="widget invoice">
                                        <div class="widget-header">
                                            ...
                                            <div class="pull-right">
                                                <p class="invoice-nr"><strong>Transfer ID:</strong> <?php echo $invid; ?></h3></p>
                                                <p class="invoice-date"><?php ?></p>
                                            </div>
                                        </div>
                                        <div class="widget-content">
                                            <div class="row">
                                                ...
                                                <div class="col-md-8 col-sm-8 col-xs-8 col-lg-8">
                                                    ...
                                                    <address>
                                                        <abbr><b>Sent By:</b></abbr> <?php echo getstaff($seldata['transferby']); ?><br>
                                                        <abbr><b>Sent From:</b></abbr> <?php echo getbranch($seldata['fromshop']); ?><br>
                                                        <abbr><b>Sent To:</b></abbr> <?php echo getbranch($seldata['shop']); ?><br>
                                                        <abbr><b>Transfer Date:</b></abbr> <?php echo $seldata['tstamp']; ?><br>
                                                        <abbr><b>Status:</b></abbr> <?php echo $seldata['status']; ?><br>
                                                    </address>
                                                </div>
                                                <?php if($seldata['status'] != "Pending"){ ?>
                                                <div class="col-md-4 col-sm-4 col-xs-4 col-lg-4 align-left">
                                                    <h3>Receipt Details</h3>

                                                    <address>
                                                        <abbr><b>Receipient:</b></abbr> <?php echo getstaff($seldata['receiveby']); ?><br>
                                                        <abbr><b>Receive Date:</b></abbr> <?php echo $seldata['receivedate']; ?><br>
                                                    </address>
                                                </div>
                                                <?php } ?>
                                                <!-- /Adresses -->
                                                ...
                                                <div class="col-md-12">
                                                    <table class="table table-striped table-hover">
                                                        <?php if($seldata['status'] == 'Pending' || $seldata['status'] == "Rejected"){ ?>
                                                        <thead>
                                                        <tr>
                                                            <th>#</th>
                                                            <th>Product ID</th>
                                                        ...
                                                        <tbody>
                                                        <?php
                                                        $seldet = "SELECT * FROM products_temp WHERE transid = '$invid'";
                                                        $seldetrun = $conn->query($dbconnection,$seldet);
                                                        $count = 0;
                                                        $subtotal = 0;
                                                        while($ddata = $conn->fetch($seldetrun)){
                                                            ...
                                                            <tr>
                                                                <td><?php echo $count; ?></td>
                                                                <td><?php echo $ddata['pid']; ?></td>
                                                                <td><?php echo getproduct($ddata['pid']); ?></td>
                                                                <td><?php echo $ddata['qty']; ?></td>
                                                            </tr>
                                                        <?php }?>
                                                        </tbody>
                                                        <?php }else{ ?>
                                                            ...
                                                            <tbody>
                                                            <?php
                                                            $seldet = "SELECT * FROM transfers WHERE transid = '$invid'";
                                                            $seldetrun = $conn->query($dbconnection,$seldet);
                                                            $count = 0;
                                                            $subtotal = 0;
                                                            while($ddata = $conn->fetch($seldetrun)){
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td><?php echo $ddata['pid']; ?></td>
                                                                    <td><?php echo getproduct($ddata['pid']); ?></td>
                                                                    <td><?php echo $ddata['prev']; ?></td>
                                                                    <td><?php echo $ddata['cur']; ?></td>
                                                                    <td><?php echo $ddata['cur'] + $ddata['prev']; ?></td>
                                                                </tr>
                                                            <?php }?>
                                                            </tbody>
                                                        <?php } ?>
                                            ...
                                            <div class="row padding-top-10px">
                                                ...
                                                <div class="col-md-4 col-sm-4 col-xs-4 align-right">
                                                    <div class="buttons">
                                                        <?php if(($seldata['status'] == "Pending" && $branch == $seldata['shop']) || ($seldata['status'] == "Pending" && $seldata['shop'] == "WH" && $access == "ADMINISTRATOR")){ ?>
                                                            &nbsp;&nbsp;<a class="btn btn-success btn-lg"  onclick="transferOps('<?php echo $invid; ?>','accept','<?php echo $stfID; ?>','<?php echo $sender; ?>','<?php echo $sendershop; ?>')">Accept Transfer <i class="icon-angle-right"></i></a>
                                                            &nbsp;&nbsp;<a class="btn btn-danger btn-lg"  onclick="transferOps('<?php echo $invid; ?>','reject','<?php echo $stfID; ?>','<?php echo $sender; ?>','<?php echo $sendershop; ?>')">Reject Transfer <i class="icon-angle-right"></i></a>
                                                        <?php } ?>
                                                    </div>
                                                </div>
                                            </div> <!-- /.row -->
                                        ...
                                        <div class="widget-content">
                                            ...
                                            <div class="row padding-top-10px">
                                                <div class="col-md-4 col-sm-4 col-xs-4">
                                                    <?php
                                                    $tottax=0;
                                                    $gettax = "SELECT taxid, dval, damount FROM quote_tax WHERE invid = '$invid'";
                                                    $gettaxrun = $conn->query($dbconnection,$gettax);
                                                    if($conn->sqlnum($gettaxrun)  != 0){
                                                        ?>
                                                        <table class="table table-bordered table-striped">
                                                ...
                                                <div class="col-md-8 align-right">
                                                    ...
                                                    <div class="buttons">
                                                        <a class="btn btn-default btn-lg" href="javascript:void(0);" onclick="javascript:window.print();"><i class="icon-print"></i> Print</a>
                                                        <?php if($seldata['status'] == "Inactive"){ ?>
                                                            &nbsp;&nbsp;<a class="btn btn-warning btn-lg"  onclick="activateQuote('<?php echo $invid; ?>')">Activate Quotation <i class="icon-angle-right"></i></a>
                                                            &nbsp;&nbsp;<a class="btn btn-primary btn-lg"  data-toggle="modal" href="#addtax2">Add Tax <i class="icon-angle-right"></i></a>
                                                        <?php }else{ ?>
                                                            <a class="btn btn-success btn-lg" onclick="validatequotation('<?php echo $invid; ?>')">Generate Invoice <i class="icon-angle-right"></i></a>
                                                        <?php } ?>
                                                    </div>
                    ...
                    <div class="modal fade" id="addtax2">
                        <div class="modal-dialog">
                            <form method="post" enctype="multipart/form-data">
                                <input type="hidden" name="branch" value="<?php echo $branch ?>" />
                                <input type="hidden" name="invoiceid" value="<?php echo $invid; ?>" />
                                    ...
                                    <div class="modal-body">
                                        <table class="table table-striped">
                                            <tr>
                                                <td align="right"><p style="font-weight: bold;">Tax <b style="color: #FF0000; font-weight: bold; font-size: large;">*</b></p></td>
                                                <td>
                                                    <select class="select2-active form-control" name="addtaxtoquote" required>
                                                        <option value="">--SELECT TAX--</option>
                                                        <?php
                                                        $sel = "SELECT taxid, val, name FROM taxconfig WHERE status = 'Active' AND taxid NOT IN( SELECT taxid FROM quote_tax WHERE invid='$invid') ORDER BY name ASC";
                                                        $selrun = $conn->query($dbconnection,$sel);
                                                        while($data = $conn->fetch($selrun)){
                                                            ?>
                                                            <option value="<?php echo $data['taxid'].'/'.$data['val']; ?>"><?php echo $data['name']."  [".$data['val']."%]"; ?></option>
                ...
                <?php $conn->close($dbconnection);}elseif(isset($_GET['invoicegenerate'])){
                    $conn=new Db_connect;
                    $dbconnection=$conn->conn();
                    ...
                    $mov = "INSERT INTO customer_invoice (invoiceid, custname, cont, invdate, duedate, salesperson, totalamount, totaltax, paid, status, shop, organ, orgcont, orgloc, orgtype)
                            SELECT '$invid', custname, cont, invdate, duedate, salesperson, totalamount, totaltax, paid, status, shop, organ, orgcont, orgloc, orgtype FROM  customer_quotation WHERE invoiceid = '$qid'";
                    ...
                    //MOVE THE TAX RECORDS
                    $movdet = "INSERT INTO invoice_tax (invid, taxid, dval, damount) SELECT '$invid', taxid, dval, damount FROM quote_tax WHERE invid = '$qid'";
                    $conn->query($dbconnection,$movdet);
                ...
                <?php $conn->close($dbconnection);}elseif(isset($_GET['users'])){
                    $conn=new Db_connect;
                    $dbconnection=$conn->conn();
                                    ...
                                    <div class="widget-content">
                                        <div class="row" style="margin: 1%">
                                            <div class="widget-content no-padding">
                                                ...
                                                <div class="form-group">
                                                    <label class="col-md-3 control-label" align="right">Search Product <span class="required">*</span></label>
                                                    <div class="col-md-9 clearfix">
                                                        <select  id="loadedproducts" name='chosen1' class='col-md-12 select2 full-width-fix required' onchange='displaysalesNew(this.value)'>
                                                            <option></option>
                                                        </select>
...
<script type="text/javascript">
    ...
    function activateQuote(invid){
        console.log(invid);
        var branch = $("#branch").val();
        var conf = confirm("Are You Sure You Want To Activate The Quotation? This action cannot be reversed");
        if (conf == true){
            $("#trans_details").modal("show");
            $.ajax({
                type: "post",
                url: "Ajax-table/ajax.php",
                data: "activateQuote="+invid+"&shop="+branch,
                success: function (data) {
                    console.log(data);
                    $("#trans_details").modal("hide");
                    var url = "dashboard.php?viewquotation=" + invid +"&branch="+branch;
                    window.location.replace(url);
    ...
    function displaysalesNew(pid){
        var user = $("#duserid").val();
        var branch = $("#branch").val();
        console.log(pid + " / "+user+" / "+branch);
        //SPLIT THE ID TO GET THE PRODUCT ID AND NAME
        var arr = pid.split(":");
        var id = arr[0];
        var pname = arr[1];
    ...
    function getReceipt(){
        ...
        var receiptNum = $("#viewslip").val();
        var branch = $("#branch").val();
        $.ajax({
            type: "post",
            url: "Ajax-table/ajax.php",
            data: "getreceipt=" + receiptNum,
            success: function (data) {
                if(data == "OK"){
                    var uRl = "dashboard.php?viewslip=" + receiptNum+"&branch="+branch;
                    location.replace(uRl);
                }else{
                    alert("Receipt Number, "+receiptNum+", Not Found");
                }
            },
            error: function (xhr, desc, err) {
                alert(err);
    ...
    function showfeedback(bname){
        ...
        console.log(arr[1]);
        if(arr[1] == "pos"){
            $("#product_loading").modal("show");
            var branch = $("#branch").val();
            var stfid = $("#duserid").val();
            //LOAD THE PRODUCTS
            $.ajax({
                type: "post",
                url: "Ajax-table/ajax.php",
                data: "loadshopproducts=" + branch + "&stfid="+stfid,
                success: function (data) {
                    $("#loadedproducts").prepend(data);
                    $("#product_loading").modal("hide");
                    console.log(data);
                },
                error: function (xhr, desc, err) {
                    alert(err);
    ...
    function shoptransfers() {
        //GET THE PRODUCTS ASSIGNED TO THE SHOP
        var counter = $("#rowcountersh").val();
        var ncounter = Number(counter) + 1;
        $("#rowcountersh").val(ncounter);
        var branch = $("#branch").val();
        if(ncounter != 0){
            $("#transhophidden").removeClass("hidden");
        }else{
            $("#transhophidden").addClass("hidden");
        }
        $.ajax({
            type: "post",
            url: "Ajax-table/ajax.php",
            data: "getproductselectsh=" + ncounter+"&branch="+branch,
            success: function (data) {
                var new_row = "<tr id='" + ncounter + "'><td>" + data + "</td><td><input class = 'form-control' type = 'number' name = 'qtysh[]' id = 'qtywh"+ncounter+"' value='1' min='1' required></td><td><a onclick='removeRowsh(" + ncounter + ")'><span class='icon icon-trash btn btn-danger'></span></a></td></tr>";
                $(".dataTables-example3").append(new_row);
                //console.log(new_row);
            },
            error: function (xhr, desc, err) {
                alert(err);
    ...
    function removeRowsh(id) {
        ...
        var parents = document.getElementById("pvbodysh");
        var childs = document.getElementById(id);
        parents.removeChild(childs);
    }
    ...
    function validatequotation(id){
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data);
                if(data == "OK"){
                    var uRl = "dashboard.php?invoicegenerate=" + id+"&branch="+branch;
                    location.replace(uRl);
                }else{
                   alert(data);
                }
